...
All five were found by running the pipeline, and **numbers 2 and 3 are the dangerous ones:
they do not fail, they silently build the wrong thing.** Both would have corrupted the
second FRO-7 run had they not been caught by hand.
...
- **FRO-7** — `blocked`, `env-gate-failed`, run `f5ce5803462d4766`. Linear is `In Progress`
  with `needs-info` (its own blocked run wrote it); `factory cancel` restores `Todo` and
  clears the label. The implementation it produced is real — 6 files, 78 unit tests, 5 e2e
  tests, all five stop gates and playwright passing — and lives in the clone at
  `state/clone/frontend-harness/FRO-7/f5ce5803462d4766/`. It will be redone from scratch.
- **FRO-6** — Done, #41 merged. Its host worktree was removed by hand; its clone branch
  remains.
...
unattended path still has not delivered one.
