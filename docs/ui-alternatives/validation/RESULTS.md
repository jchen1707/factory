...
Review follow-up: the complete browser checker also passed after adding Existing model preset, Repository default delivery profile, Inherit delegation and blank inherited capacity inputs. The checker explicitly selects these options and clears the three capacity values, then verifies form validity. Agent adapter selection remains a CLI capability and is not presented as an existing console setting. The factory gate report predates this fixture-only follow-up; the browser checks cover the revised behavior.
