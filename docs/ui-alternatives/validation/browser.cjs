...
if(['projects','settings'].includes(view)){await page.locator('select[name=preset]').selectOption({label:'Existing'});await page.locator('select[name=profile]').selectOption({label:'Repository default'});for(const label of ['Active agent limit','Children per parent','Delegation depth'])await page.getByLabel(label,{exact:true}).fill('');await page.locator('select').filter({has:page.locator('option', {hasText:'Inherit'})}).last().selectOption({label:'Inherit'});if(!await page.locator('select[name=preset]').evaluate(el=>el.form.checkValidity()))failures.push('inherited settings rejected');}
