...
from factory import cli, driver, recovery, repo
...
def test_the_drive_calls_review_then_deliver_after_verify(
    ...
    """`driver.drive` walks the whole chain. A dry run records every would-print without
    spending a model call, which is the cheapest proof that the wiring is in place."""
    ctx.dry_run = True
    ...
    driver.drive(ctx, follow=True, poll=0)
