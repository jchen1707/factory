...
export function renderFixtureReference({name, fixture}) {
  ...
  if (name === 'projects') content = panel('Registered projects', table(['Project', 'Delivery', 'Run slots', 'Agent slots', 'Waiting'], fixture.projects.map(p => [esc(p.name), esc(p.settings.delivery_profile || p.delivery_profile), `${p.occupied_slots} / ${p.concurrency}`, `${p.active_agents} / ${p.effective_agent_limit ?? 'inherited'}`, esc(p.waiting)]))) + panel(`Project defaults · ${fixture.projects[0].name}`, settings(fixture.projects[0].settings, fixture.projects[0]));
